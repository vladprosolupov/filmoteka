...
@Component("GetPropDirectorTask")
public class GetPropDirectorTask implements Callable<Set<FilmDb>> {
    ...
    private static final Logger log = LogManager.getLogger(GetPropDirectorTask.class);

    public GetPropDirectorTask(SessionFactory sessionFactory) {
    ...
    public Set<FilmDb> call() throws Exception {
        ...
        for (Map.Entry<Integer, Integer> entry : percentage.entrySet()) {
            filmDbSet.addAll(aiService.getPropDirector(entry.getValue(), entry.getKey(), currentClient));
        }
