...
public class FilmService {
    ...
    public List<FilmJSONIndex> getFilmsForIndexPage(int page) throws HibernateException {
        ...
        session.beginTransaction();
        List<FilmJSONIndex> list = session.createQuery("select F.title, F.releaseDate, F.cover, F.id, F.rating from FilmDb F order by F.rating desc, F.releaseDate desc").setFirstResult(start).setMaxResults(limit).list();
        session.getTransaction().commit();
        session.close();
